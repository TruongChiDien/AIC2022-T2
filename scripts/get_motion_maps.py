...
with open("data/AIC21_Track5_NL_Retrieval/test_tracks.json") as f:
    tracks_test = json.load(f)
with open("data/AIC21_Track5_NL_Retrieval/train_tracks.json") as f:
...
def get_motion_map(info):
    ...
    for i in range(len(track["frames"])):
        ...
        box = track["boxes"][i]
        if i ==0:
            example = np.zeros(frame.shape,np.int32)
        if i%7==1:
